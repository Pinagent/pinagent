 ...
 * `react-native/Libraries/Inspector/getInspectorDataForViewAtPoint`, which
 * is only present in a real RN dev runtime. This package intentionally does
 ...
 * tap handler, and resolve to a safe `{ loc: null, nameChain: [], frame: null }`
...
import { resolvePick } from '../src/native/inspector';
...
describe('resolvePick — graceful degradation when the RN inspector is unavailable', () => {
  it('resolves to a safe null pick instead of throwing', async () => {
    ...
    expect(result).toEqual({ loc: null, nameChain: [], frame: null });
  });
  ...
  it('returns the same safe shape on repeated calls (memoised unavailability)', async () => {
    ...
    expect(a).toEqual({ loc: null, nameChain: [], frame: null });
    expect(b).toEqual(a);
  });
});
