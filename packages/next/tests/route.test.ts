 ...
 *
 * Setup:
 *   - PINPOINT_PROJECT_ROOT → fresh tmp dir per file so the SQLite
 *     migrations apply against an isolated DB.
 *   - PINPOINT_SPAWN_AGENT='off' so route.ts's top-level block
...
const PROJECT_ROOT = join(tmpdir(), `pp-route-${nanoid(8)}`);

beforeAll(async () => {
  process.env.PINPOINT_PROJECT_ROOT = PROJECT_ROOT;
  process.env.PINPOINT_SPAWN_AGENT = 'off';
...
afterAll(async () => {
  ...
  delete process.env.PINPOINT_PROJECT_ROOT;
...
describe('GET /widget.js', () => {
  it('returns the widget IIFE bundle with a config prelude', async () => {
    const res = await route.GET(makeRequest('/__pinpoint/widget.js'), ctx(['widget.js']));
    expect(res.status).toBe(200);
    expect(res.headers.get('content-type')).toMatch(/application\/javascript/);
    const body = await res.text();
    // Prelude inlines window.__pinpointConfig.
    expect(body).toContain('__pinpointConfig');
    // IIFE body present (the var name our bundler exposes).
    expect(body).toContain('PinpointWidget');
  });
});
...
describe('GET /sqlite-wasm/*', () => {
  it('serves sqlite3.wasm with the wasm mime', async () => {
    const res = await route.GET(
      makeRequest('/__pinpoint/sqlite-wasm/sqlite3.wasm'),
  ...
  it('serves the bundler-friendly worker entry', async () => {
    const res = await route.GET(
      makeRequest('/__pinpoint/sqlite-wasm/sqlite3-worker1-bundler-friendly.mjs'),
  ...
  it('404s for any file outside the whitelist (path traversal blocked)', async () => {
    const res = await route.GET(
      makeRequest('/__pinpoint/sqlite-wasm/random.js'),
...
describe('GET /db-worker.js', () => {
  it('serves our custom SAH-Pool-aware worker source', async () => {
    const res = await route.GET(
      makeRequest('/__pinpoint/db-worker.js'),
      ctx(['db-worker.js']),
    );
    expect(res.status).toBe(200);
    expect(res.headers.get('content-type')).toMatch(/application\/javascript/);
...
describe('GET /feedback', () => {
  it('returns an empty list when no feedback exists', async () => {
    const res = await route.GET(makeRequest('/__pinpoint/feedback'), ctx(['feedback']));
    expect(res.status).toBe(200);
...
describe('POST /feedback', () => {
  ...
  afterEach(async () => {
    ...
    for (const id of createdIds) {
      await route.PATCH(
        makeRequest(`/__pinpoint/feedback/${id}`, {
          method: 'PATCH',
  ...
  it('creates a feedback record and returns { id, agentSpawned: false }', async () => {
    const res = await route.POST(
      makeRequest('/__pinpoint/feedback', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
    ...
    // We forced PINPOINT_SPAWN_AGENT='off' so this MUST be false.
...
describe('GET /feedback/:id', () => {
  ...
  it('returns 404 for a non-existent id', async () => {
    const res = await route.GET(
      makeRequest('/__pinpoint/feedback/aBcDeFgHiJ'),
...
describe('POST /open', () => {
  it('returns 400 when file is missing', async () => {
    const res = await route.POST(
      makeRequest('/__pinpoint/open', { method: 'POST' }),
      ctx(['open']),
    );
    expect(res.status).toBe(400);
  });
...
describe('404 fallthrough', () => {
  it('GET unknown slug → 404', async () => {
    const res = await route.GET(
      makeRequest('/__pinpoint/random-nothing'),
      ctx(['random-nothing']),
    );
    expect(res.status).toBe(404);
  });
  ...
  it('POST unknown slug → 404', async () => {
    const res = await route.POST(
      makeRequest('/__pinpoint/random-nothing', { method: 'POST' }),
      ctx(['random-nothing']),
    );
    expect(res.status).toBe(404);
  });
  ...
  it('PATCH unknown slug → 404', async () => {
    const res = await route.PATCH(
      makeRequest('/__pinpoint/random-nothing', { method: 'PATCH' }),
      ctx(['random-nothing']),
    );
    expect(res.status).toBe(404);
  });
