...
describe('session token round-trip', () => {
  it('signs and verifies, preserving claims', async () => {
    const token = await signSessionToken({ tenantId: 'acme', sessionId: 'sess-1' }, SECRET, {
      nowSeconds: 1000,
      ttlSeconds: 3600,
    });
  ...
  it('round-trips values needing base64url-safe encoding', async () => {
    const token = await signSessionToken(
      { tenantId: 'org/with+slash', sessionId: 'sömé-üñïçødé' },
      SECRET,
    );
...
describe('session token rejection', () => {
  ...
  it('rejects a forged payload carried under a stale signature', async () => {
    ...
    const honest = await signSessionToken({ tenantId: 'acme', sessionId: 'sess-1' }, SECRET);
    const evil = await signSessionToken({ tenantId: 'evil', sessionId: 'sess-1' }, SECRET);
