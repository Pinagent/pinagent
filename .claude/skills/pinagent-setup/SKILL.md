...
- **Auto mode blocks MCP tools by default.** The agent will say "tools were denied" unless the project's `.claude/settings.local.json` allow-lists `mcp__pinagent__*`. Use the `/permissions` slash command inside Claude Code to set this up. **In a multi-app monorepo each app has its own server key** (`pinagent-mobile`, `pinagent-www`, …) and a permission rule's server segment is glob-free — so allow-list **every** key (`mcp__pinagent__*`, `mcp__pinagent-mobile__*`, …), not just `mcp__pinagent__*`. Miss one and that app's feedback silently auto-denies in spawn/channel runs ("the … channel needs an interactive permission grant that I can't self-approve"). See [mcp.md](./mcp.md) §4.
