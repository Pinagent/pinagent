...
The plugins, MCP server, and CLI are published to npm under the `@pinagent/*` scope — `@pinagent/vite-plugin`, `@pinagent/next-plugin`, `@pinagent/mcp`, and `@pinagent/cli` (the `pinagent` command: `init`, `mcp`, `transcript`). Install steps use `pnpm add -D` and `pnpm dlx`; nothing references a local checkout, so the skill works in any project.
