...
> - **Phase B (done, WebSocket).** Per-feedback in-memory event bus (`packages/next/src/event-bus.ts`) fans SDK events to subscribers. A dedicated WebSocket server (`packages/next/src/ws-server.ts`, default port 53636 — env override `PINAGENT_WS_PORT`) handles all widget ↔ dev bidirectional traffic: event subscriptions, follow-up `user_message`s, `ask_response` replies, and `interrupt`. The widget bundle's prelude inlines the WS URL at serve time.
